...
lookUp :: Eq a => a -> [(a, b)] -> b
lookUp x xys = head [y | (a, y) <- xys, a == x]

checkSat :: BDD -> Env -> Bool
checkSat (0, _) _ = False
checkSat (1, _) _ = True
checkSat (root, rest) env
  | x == True  = checkSat (r, rest) env
  | x == False = checkSat (l, rest) env
  where
    (i, l, r) = lookUp root rest
    x = lookUp i env

sat :: BDD -> [[(Index, Bool)]]
sat (0, _)       = []
sat (1, _)       = [[]]
sat (root, rest) = [(i, False) : x | x <- sat (l, rest)] ++ [(i, True): x | x <- sat (r, rest)]
  where (i, l, r) = lookUp root rest 
...
simplify (And (Prim a) (Prim b))
  | a == True && b == True = Prim True
  | otherwise = Prim False
simplify (Or (Prim a) (Prim b))
  | a == False && b == False = Prim False
  | otherwise = Prim True
simplify exp = exp

restrict :: BExp -> Index -> Bool -> BExp
restrict e@(Prim  b) _ _ = e
restrict e@(IdRef x) i v
  | x == i    = Prim v
  | otherwise = e
restrict (Not e) i v     = simplify (Not (restrict e i v))
restrict (Or e1 e2) i v  = simplify (Or (restrict e1 i v) (restrict e2 i v))  
restrict (And e1 e2) i v = simplify (And (restrict e1 i v) (restrict e2 i v))
...
buildBDD' (Prim True)  i  []      = (1, [])
buildBDD' (Prim False) i  []      = (0, [])
buildBDD' e            i  (x:xs)  = (i, (i, (x, l', r')) : (l ++ r))
 where (l',l) = buildBDD' (restrict e x False) (i * 2) xs
       (r',r) = buildBDD' (restrict e x True)  (i * 2 + 1) xs  
