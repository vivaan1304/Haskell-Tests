...
-- update x [] = [x]
-- update x@(a_,b_) ((a, b): abs) 
--  | a_ == a = (a, b_) : abs
--  | a_ /= a = (a, b)  : update x abs 
update (x, y) st = (x, y) : filter ((/= x) . fst) st
...
-- Const Int | Var Id | Apply Op Exp Exp
eval (Const x) _       = x
...
execStatement (Assign i e) s  = update (i, eval e s) s
execStatement (If p q r)   s
 | eval p s == 1 = execBlock q s
 | otherwise     = execBlock r s
execStatement statement@(DoWhile b p) s
 | eval p s' == 1 = execStatement statement s'
 | otherwise      = s'
 where s' = execBlock b s

execBlock :: Block -> State -> State
-- execBlock []       st = st
-- execBlock (s : ss) st = execBlock ss (execStatement s st)
execBlock b st = foldl (flip execStatement) st b
------------------------------------------------------------------------
...
foldConst e@(Phi c@(Const c1) (Const c2))
 | c1 == c2 = c
--  | c1 /= c2 = e
foldConst (Apply o (Const c1) (Const c2)) = Const (apply o c1 c2)
...
sub :: Id -> Int -> Exp -> Exp
-- Pre: the expression is in SSA form
sub i x e = foldConst (sub' i x e)
 where
    -- Pre: the expression is in SSA form
    -- Const Int | Var Id | Apply Op Exp Exp
    -- basically substitutes in the given expression
    sub' :: Id -> Int -> Exp -> Exp
    sub' i x e@(Apply o e1 e2)
     | e1 == (Var i) && e2 == (Var i) = Apply o (Const x) (Const x)
     | e1 == (Var i)                  = Apply o (Const x) e2
     | e2 == (Var i)                  = Apply o e1        (Const x)
     | otherwise                      = e
    sub' i x e@(Var v)
     | i == v = Const x
     | i /= v = e
    sub' i x e@(Phi e1 e2)
     | e1 == (Var i) && e2 == (Var i) = Phi (Const x) (Const x)
     | e1 == (Var i)                  = Phi (Const x) e2
     | e2 == (Var i)                  = Phi e1        (Const x)
     | otherwise                      = e
    sub' _ _ e = e
...
-- DoWhile Block Exp 
scan _ _ []                          = ([],[])
scan i v ((Assign "$return" e) : ss) = let (worklist, block) = scan i v ss in (worklist, (Assign "$return" (sub i v e)) : block)
scan i v ((Assign x (Const c)) : ss) = let (worklist, block) = scan i v ss in ((x, c) : worklist, block)   -- x = 5
scan i v (s@(Assign x (Var c)) : ss)     -- x = c TODO
 | c == i = let (worklist, block) = scan i v ss in ((x, v) : worklist, block)
 | c /= i = let (worklist, block) = scan i v ss in (worklist, s : block)
scan i v (s@(Assign x e@(Apply o e1 e2)) : ss) -- x = e1 <o> e2 TODO
 | e == rhs = let (worklist, block) = scan i v ss in (worklist, s : block)
 | e /= rhs = scan i v ((Assign x rhs) : ss)
 where rhs = sub i v e
scan i v ((Assign x e@(Phi e1 e2)) : ss) = (worklist, (Assign x (sub i v e)) : block) -- x = PHI(e1, e2)
 where (worklist, block) = scan i v ss
scan i v ((If p q r) : ss) = (worklist' ++ worklist'' ++ worklist, (If p block' block'') : block)
 where (worklist, block)     = scan i v ss
       (worklist', block')   = scan i v q
       (worklist'', block'') = scan i v r
scan i v ((DoWhile body p) : ss) = (worklist' ++ worklist, (DoWhile block' p) : ss)
 where (worklist, block)   = scan i v ss
       (worklist', block') = scan i v body 
scan i v (s : ss) = let (wl, bl) = scan i v ss in (wl, s : bl)
...
-- propagateConstants _ = undefined
propagateConstants b = propagateConstants' wl_i bl_i
 where
  (wl_i, bl_i) = scan "$INVALID" 0 b
  propagateConstants' :: Worklist -> Block -> Block
  propagateConstants' [] block = block
  propagateConstants' ((i, v) : ws) block = let (new_w, block') = scan i v block in propagateConstants' (new_w ++ ws) block'
------------------------------------------------------------------------
...
-- Pre: the block is in SSA form
unPhi [] = []
unPhi ((If p q r) : (Assign v (Phi e1 e2)) : b)         = unPhi ((If p ((unPhi q) ++ [Assign v e1]) ((unPhi r) ++ [Assign v e2])) : b)
unPhi ((DoWhile ((Assign v (Phi e1 e2)) : body) p) : b) = ( (Assign v e1) : unPhi [DoWhile (body  ++ [Assign v e2]) p] ) ++ unPhi b
unPhi ((If p q r) : b)                                  = (If p (unPhi q) (unPhi r)) : unPhi b
unPhi ((DoWhile body p) : b)                            = (DoWhile (unPhi body) p)   : unPhi b
unPhi (s : b) = s : unPhi b
------------------------------------------------------------------------
-- Part IV
